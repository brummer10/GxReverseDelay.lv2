...
static void resize_event(gx_reversedelayUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_reversedelayUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_reversedelayUI* ui = (gx_reversedelayUI*)malloc(sizeof(gx_reversedelayUI));
	...
	ui->width = ui->init_width -140 + (100 * CONTROLS);
	...
	ui->frame = cairo_image_surface_create (CAIRO_FORMAT_ARGB32, 81, 81);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width-140 + (100 * CONTROLS));
...
static void cleanup(LV2UI_Handle handle) {
	gx_reversedelayUI* ui = (gx_reversedelayUI*)handle;
	cairo_destroy(ui->cr);
...
static void indicator_expose(gx_reversedelayUI *ui, gx_controller* knob)
{
	...
	double x0      = 5.0;
	double y0      = 10.0;
	double rect_width  = w-10;
	double rect_height = h-20;
	...
	cairo_pattern_t* 	pat2 = cairo_pattern_create_linear (x0+rect_width/2, y0,x0+rect_width/2,rect_height+5);
	cairo_pattern_add_color_stop_rgba (pat2, 0,  0.2, 0.2, 0.2, 1.0);
	cairo_pattern_add_color_stop_rgba (pat2, 0.2,  0.1, 0.1, 0.1, 1.0);
	cairo_pattern_add_color_stop_rgba (pat2, 0.5,  0.2, 0.2+0.2*knobstate, 0.2, 1.0);
	cairo_pattern_add_color_stop_rgba (pat2, 0.8,  0.1, 0.1, 0.1, 1.0);
	cairo_pattern_add_color_stop_rgba (pat2, 1,  0.4+0.6*(1-knobstate), 0.4, 0.4, 1.0);

	// base
	rounded_rectangle(ui->crf, x0, y0, x1, y1);
	cairo_set_source (ui->crf, pat);
	cairo_fill_preserve (ui->crf);
	cairo_set_source_rgba (ui->crf, 0.1, 0.1, 0.1, 1.0);
	cairo_set_line_width (ui->crf, 2.0);
	cairo_stroke (ui->crf);
	...
	cairo_move_to (ui->crf, (rect_width+5)/2-extents.width/2, grow+6+extents.height);
	cairo_show_text(ui->crf, knob->label);
	cairo_new_path (ui->crf);
}

// draw knobs and simple switches
static void knob_expose(gx_reversedelayUI *ui,gx_controller* knob) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void bypass_expose(gx_reversedelayUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(gx_reversedelayUI *ui, gx_controller* controller) {
	...
	else if (controller->type == INDICATOR) indicator_expose(ui, controller);
}

// general XWindow expose callback, 
static void _expose(gx_reversedelayUI *ui) {
	static const char* plug_name = "GxReverseDelay" ;
...
static void controller_expose(gx_reversedelayUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
	...
	cairo_rectangle (ui->cr,(double)control->al.x * ui->rescale.x2,
	  ...
	  (double)control->al.width+39.0, (double)control->al.height+20.0);
	cairo_fill(ui->cr);
	cairo_stroke(ui->cr);
...
static void send_controller_event(gx_reversedelayUI *ui, int controller) {
...
static void check_value_changed(gx_reversedelayUI *ui, int i, float* value) {
...
static void check_is_active(gx_reversedelayUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_reversedelayUI *ui) {
...
static bool get_active_ctl_num(gx_reversedelayUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_reversedelayUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_reversedelayUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_reversedelayUI *ui, int i) {
	float value;
...
static void motion_event(gx_reversedelayUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_reversedelayUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_reversedelayUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_reversedelayUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_reversedelayUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_reversedelayUI *ui, bool set) {
...
static void event_handler(gx_reversedelayUI *ui) {
	XEvent xev;
